...
def test_mlp_c1() -> None:
    torch.manual_seed(2)
    in_dim = 10
    h_dim = 32
    out_dim = 2

    l1_w = q88_from_float_tensor(
        torch.rand((h_dim, in_dim), dtype=torch.float32) * 2.0 - 1.0
    )
    l2_w = q88_from_float_tensor(
        torch.rand((h_dim, h_dim), dtype=torch.float32) * 2.0 - 1.0
    )
    l3_w = q88_from_float_tensor(
        torch.rand((out_dim, h_dim), dtype=torch.float32) * 2.0 - 1.0
    )
    x_q = q88_from_float_tensor(
        torch.rand((1, in_dim), dtype=torch.float32) * 2.0 - 1.0
    )
    ...
    build_dir = REPO_ROOT / "tests" / "build" / "mlp_c1"
    ...
    tb_path = build_dir / "tb_mlp_c1.sv"
